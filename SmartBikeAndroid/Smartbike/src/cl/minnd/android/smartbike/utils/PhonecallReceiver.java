		...
		public void onCallStateChanged(int state, String incomingNumber) {
			...
			case TelephonyManager.CALL_STATE_RINGING:
				...
				if (callListener != null)
					callListener.onIncomingCallStarted(savedNumber,
							callStartTime);
				break;
			case TelephonyManager.CALL_STATE_OFFHOOK:
				...
				if (lastState != TelephonyManager.CALL_STATE_RINGING) {
					...
					callStartTime = new Date();
					if (callListener != null)
						callListener.onOutgoingCallStarted(savedNumber,
								callStartTime);
				}
				break;
			case TelephonyManager.CALL_STATE_IDLE:
				...
				if (lastState == TelephonyManager.CALL_STATE_RINGING) {
					...
					if (callListener != null)
						callListener.onMissedCall(savedNumber, callStartTime);
				} else if (isIncoming) {
					if (callListener != null)
						callListener.onIncomingCallEnded(savedNumber,
								callStartTime, new Date());
				} else {
					if (callListener != null)
						callListener.onOutgoingCallEnded(savedNumber,
								callStartTime, new Date());
				}
				break;
			}
